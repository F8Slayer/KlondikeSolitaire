...
public class UserInput : MonoBehaviour
{
    ...
    void Card(GameObject selected)
    {
        ...
        if (slot1 == this.gameObject) // not null beacuse we pass in this gameObject instead
        {
            slot1 = selected;
        }

        // if the card is already a card selected (and it is not the same card)
    ...
    void Top(GameObject selected)
    {
        ...
        {
            // if the card is an ace and the emty slot is top then stack
    ...
    void Bottom(GameObject selected)
    {
        ...
        // if the card is a king and the emty slot is bottom then stack
    ...
    bool Stackable(GameObject selected)
    {
        ...
        if (!s2.inDeckPile)
        {
            if (s2.top) // if in the topppile must stuck suited Ace to King
            {
    ...
    {
        // if on top of king or emty bottom stack the cards in place
        ...
        s1.inDeckPile = false; // you cannotadd cards to the top trips pile so this is always fine
        s1.row = s2.row;
        ...
        // after comleting move reset slot1 to be essentually null as being null will break the logic
        slot1 = this.gameObject;
    }
    ...
    void AutoStack(GameObject selected)
    {
        for (int i = 0; i < solitaire.topPos.Length; i++)
        {
            ...
            if (selected.GetComponent<Selectable>().value == 1) // if it is and Ace
            {
            ...
            else
            {
                if ((solitaire.topPos[i].GetComponent<Selectable>().suit == slot1.GetComponent<Selectable>().suit) && (solitaire.topPos[i].GetComponent<Selectable>().value == slot1.GetComponent<Selectable>().value - 1))
                {
                    ...
                    {
                        slot1 = selected;
                        // find a top spot the matches the conditions for auto stacking if it exists
